...
public class TimelineService(OracleDbContext db) : ServiceBase(db)
{
    ...
    public List<TimelineDateVm> AssembleTimelineVms(List<int> characterIds, List<CharacterTimeline> timelineData, int startDate, int endDate)
    {
	    ...
	    foreach (var characterId in characterIds)
	    {
		    for (int i = startDate; i <= endDate; i++)
		    {
			    ...
			    var timelineEvent = timelineData.FirstOrDefault(x => x.CharacterId == characterId
			                                                         && i >= x.StartDay && (x.EndDay == null || i <= x.EndDay));
